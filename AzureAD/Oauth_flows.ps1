...
## CURL
curl.exe -X POST -H "Content-Type: application/x-www-form-urlencoded" -d "client_id=<client-id>&scope=https%3A%2F%2Fgraph.microsoft.com%2F.default&username=<complete-upn>&password=<psd>&grant_type=password" --url "https://login.microsoftonline.com/{tenant-id}/oauth2/v2.0/token"
...
In Case of ROPC, the calling User needs to have sufficient permissions (Role) granted in Azure AD

For example, in this case We are trying to get other user details and to get the intended data, We need below two permissions. If any missing, data can't be retreived
  1. User has Programatic Read permissions Role on the Azure AD e.g. User Administrator (Least Access Priv model should be followed)
  2. Client ID has "User.read.all" Delegated API Permission
